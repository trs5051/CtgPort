@extends('layouts.admin-master')
@section('admin-sidebar')
    @include('layouts.admin-sidebar')
@endsection
@section('content')
    <div class="content-area" style="padding-top: 15px;">
        <div class="container-fluid  pl-0 pr-0" >
            <div class="panel panel-default">

            <div class="panel-heading ivr-heading" style="overflow: hidden;">

                <h3 style="float: left;" id="inv_title">Invoice Report</h3>                

                <button id="ctg-print-button" class="ctg-p-button inv-report-p" onclick="PrintIt()" style="display: inline-block;">print</button>

                <div class="invoice_date_filter" style="float: right; margin-right:20px; ">
                <form action="/search/invoice/report" method="post">
                    {{csrf_field()}}
                    <label> Sticker: </label>
                  <select name="sticker_type" id="sticker_type" class="form-control-sm " >
                         
                          <option selected="" value="{{!empty($sticker_type)?$sticker_type->id:''}}">{{!empty($sticker_type)?$sticker_type->name:'Select One'}}</option>

                             <?php $sticker_types=App\StickerCategory::all(); ?>
                                @if(isset($sticker_types))
                                <option value="" readonly>Select One</option>
                                    @foreach($sticker_types as $sticker_type)
                                
                                <option value="{{$sticker_type->id}}">{{$sticker_type->name}}</option>
                                    @endforeach
                                @endif
                  </select> {{csrf_field()}}

                    <label> Vehicle: </label> 
                     <select name="vehicle_type" id="vehicle_type" class="form-control-sm " >

                          <option selected="" value="{{!empty($vehicle_type)?$vehicle_type->id:''}}">{{!empty($vehicle_type)?$vehicle_type->name:'Select One'}}</option>

                             <?php $vehicleTypes=App\VehicleType::all(); ?>
                                @if(isset($vehicleTypes))
                                <option value="" readonly>Select One</option>

                                    @foreach($vehicleTypes as $vehicleType)
                                <option value="{{$vehicleType->id}}">{{$vehicleType->name}}</option>
                                    @endforeach
                                @endif
                    </select>
                <label> From: </label> <input style="border: none; padding: 2px 5px;" class="form-control-sm" type="date" name="start_inv_date" value="{{isset($date_from) ? $date_from : ''}}">
                  <label>To:</label> <input class="form-control-sm" type="date" style="border: none; padding: 2px 5px;" name="end_inv_date" value="{{isset($date_to) ? $date_to : ''}}">
                  <button type="submit" style="border: none;padding: 3px 10px;border-radius: 4px;font-size: 14px;" class="btn-info">Show Invoice</button> 
                    </form>
                </div>  

             </div>
             <div class="panel-body" style="padding:10px 0;" >

                <div id="example-wrapper" >           

                    <table id="example1" class="table table-bordered dt-responsive">                        
                        <thead>
                            <tr>
                                <th scope="col" style='text-align: center;'>SI. No.</th>
                                <th scope="col" style='text-align: center;'>Vehicle Type</th>
                                <th scope="col" style='text-align: center;'>No. of Issuance</th>
                                <th scope="col" style='text-align: center;'>Total Amount Collected</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php $countVehicle=0; $countTotal=0;  ?>

                        @if(isset($invoices) &&  $invoices !='')
                         @foreach($invoices as $key => $invoice)
                        <tr class='invoice-report-row'>
                           <th class='serial' scope="row" style='text-align: center;'>{{$loop->iteration}}</th>

                            <td  class='vehicleType' style='text-align: center;'> {{$invoice->vehicleType->name}}</td>

                            <td  class="no-of-issuance" style='text-align: center;'>
                                
                                    {{count(App\Invoice::where('vehicle_type_id', $invoice->vehicle_type_id)->get())}}
                                    <?php $countVehicle+=count(App\Invoice::where('vehicle_type_id', $invoice->vehicle_type_id)->get()); ?>
                                
                            </td>
                            <td class="total-ammount" style='text-align: center;'> {{sprintf('%0.2f',App\Invoice::where('vehicle_type_id', $invoice->vehicle_type_id)->sum('total'))}}
                                    <?php $countTotal+=sprintf('%0.2f',App\Invoice::where('vehicle_type_id', $invoice->vehicle_type_id)->sum('total')) ?>

                            </td>
                        
                           
                        </tr>
                        @endforeach
                        <tr>
                            <th colspan="" style="text-align: center;"> Total</th>
                            <th ></th>
                            <th style="text-align: center;">{{$countVehicle}}</th>
                            <th style="text-align: center;" clsss="countTotal">{{sprintf('%0.2f', $countTotal)}}</th>
                        </tr>
                        @endif

                        </tbody>
                    </table>
                    
                    <hr> 

                    <table id="example2" class="table table-bordered dt-responsive mt-4">

                        <thead style='text-align: center;'>
                        <tr>
                            <th scope="col">Total</th>
                            <th scope="col">Pass Issued</th>
                            <th scope="col">Revenue</th>
                            <th scope="col">CPA Fund</th>
                            <th scope="col">Vat</th>
                        </tr>
                        </thead>

                        <tbody style='text-align: center;'>
                            <tr>
                                <td></td>
                                <td>{{$countVehicle > 0 ? $countVehicle : '0.00'}}</td>
                                <td>{{$countTotal > 0 ? sprintf('%0.2f',$countTotal) : '0.00'}}</td>
                                <td>{{$countTotal > 0 ? sprintf('%0.2f',$countTotal/1.15) : '0.00'}}</td>
                                <td>{{$countTotal > 0 ? sprintf('%0.2f',$countTotal - $countTotal/1.15) : '0.00'}}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>

       </div>
    </div>
@endsection

@section('admin-script')
<script>

$(document).ready(function() {
    // Datatable1 
    var table = $('#example1').DataTable( {
    } );

    table.buttons().container().appendTo( '.panel-heading' );

    // Datatable2 
    var table = $('#example2').DataTable( {
    } );

    table.buttons().container().appendTo( '.panel-heading' );

    // If data table empty
    var inv_countTotal = parseFloat('{{$countTotal}}').toFixed('2');
    if( inv_countTotal == "0.00" ){ 
        $('.inv-report-p').addClass('disabled').attr("disabled", 'disabled');
    }

} );

var words_string='';
function convertNumberToWords(amount,words_string) {
    var words = new Array();
    words[0] = '';
    words[1] = 'One';
    words[2] = 'Two';
    words[3] = 'Three';
    words[4] = 'Four';
    words[5] = 'Five';
    words[6] = 'Six';
    words[7] = 'Seven';
    words[8] = 'Eight';
    words[9] = 'Nine';
    words[10] = 'Ten';
    words[11] = 'Eleven';
    words[12] = 'Twelve';
    words[13] = 'Thirteen';
    words[14] = 'Fourteen';
    words[15] = 'Fifteen';
    words[16] = 'Sixteen';
    words[17] = 'Seventeen';
    words[18] = 'Eighteen';
    words[19] = 'Nineteen';
    words[20] = 'Twenty';
    words[30] = 'Thirty';
    words[40] = 'Forty';
    words[50] = 'Fifty';
    words[60] = 'Sixty';
    words[70] = 'Seventy';
    words[80] = 'Eighty';
    words[90] = 'Ninety';
    
    var atemp = amount.toString().split(".");

    var number = atemp[0].split(",").join("");
    var decimal = atemp[1].split(",").join(""); 

    var n_length = number.length;
    if (n_length <= 9) {
        var n_array = new Array(0, 0, 0, 0, 0, 0, 0, 0, 0);
        var received_n_array = new Array();
        for (var i = 0; i < n_length; i++) {
            received_n_array[i] = number.substr(i, 1);
        }
        for (var i = 9 - n_length, j = 0; i < 9; i++, j++) {
            n_array[i] = received_n_array[j];
        }
        for (var i = 0, j = 1; i < 9; i++, j++) {
            if (i == 0 || i == 2 || i == 4 || i == 7) {
                if (n_array[i] == 1) {
                    n_array[j] = 10 + parseInt(n_array[j]);
                    n_array[i] = 0;
                }
            }
        }
        value = "";
        for (var i = 0; i < 9; i++) {
            if (i == 0 || i == 2 || i == 4 || i == 7) {
                value = n_array[i] * 10;
            } else {
                value = n_array[i];
            }
            if (value != 0) {
                words_string += words[value] + " ";
            }
            if ((i == 1 && value != 0) || (i == 0 && value != 0 && n_array[i + 1] == 0)) {
                words_string += "Crores ";
            }
            if ((i == 3 && value != 0) || (i == 2 && value != 0 && n_array[i + 1] == 0)) {
                words_string += "Lakhs ";
            }
            if ((i == 5 && value != 0) || (i == 4 && value != 0 && n_array[i + 1] == 0)) {
                words_string += "Thousand ";
            }
            if (i == 6 && value != 0 && (n_array[i + 1] != 0 && n_array[i + 2] != 0)) {
                words_string += "Hundred ";
            } else if (i == 6 && value != 0) {
                words_string += "Hundred ";
            }
        }
    }

    var d_length = decimal.length;
    if (d_length <= 9) {
        var d_array = new Array(0, 0, 0, 0, 0, 0, 0, 0, 0);
        var received_n_array = new Array();
        for (var i = 0; i < d_length; i++) {
            received_n_array[i] = decimal.substr(i, 1);
        }
        for (var i = 9 - d_length, j = 0; i < 9; i++, j++) {
            d_array[i] = received_n_array[j];
        }
        for (var i = 0, j = 1; i < 9; i++, j++) {
            if (i == 0 || i == 2 || i == 4 || i == 7) {
                if (d_array[i] == 1) {
                    d_array[j] = 10 + parseInt(d_array[j]);
                    d_array[i] = 0;
                }
            }
        }
        value = "";
        var dec_string='';
        for (var i = 0; i < 9; i++) {
            if (i == 0 || i == 2 || i == 4 || i == 7) {
                value = d_array[i] * 10;
            } else {
                value = d_array[i];
            }
            if (value != 0) {
                dec_string += words[value] + " Paisa";
            }

        }
    }
     words_string = words_string.split("  ").join(" ");
     if(dec_string!=''){
             return words_string+'Taka And '+dec_string;

         }else{

             return words_string+'Taka And Zero Paisa';
         }
}

function PrintIt() {

    var amount=parseFloat('{{$countTotal}}').toFixed('2');    
    var mywindow = window.open('', 'PRINT');
    var is_chrome = Boolean(mywindow.chrome);

    mywindow.document.write('<html><head style="padding:0; margin:0 auto"><title>Invoice Report</title>');
    
    mywindow.document.write('<style>table{width: 100%;font-size:14px;border-collapse:collapse;margin-bottom:15px;}table th,table td{padding:5px 10px;text-aligh:center;border:1px solid #ddd;}.footersign span{border-top:1px solid black;padding-top:5px;} table#example2{margin-bottom:15px} #adm-logo,.adm-logo{height:65px;width:65px;border-radius:50%; float:left}#adm-logo.left{float:left;background-color: #ddd;}#adm-logo.right{float:right;background-color: transparent;} </style>');

    mywindow.document.write('</head><body>');

    mywindow.document.write('<div class="header" style="display:flex;justify-content:space-between;align-items:center;">');
    mywindow.document.write('<div id="adm-logo" class="left"><img width="65" src="data:image/png;base64," class="img-fluid" alt="logo"></div>');
    mywindow.document.write('<div><h3 style="text-align: center;font-size: 16px;margin-top:-4px;">Chittagong Port Authority <br/>Office of the Director (Security) <br/>Bandar, Chittagong-4100</h3></div>');
    mywindow.document.write('<div class="adm-logo right"></div>');
    mywindow.document.write('</div>');
    mywindow.document.write('<h3 style="text-align: center; font-size: 15px; margin:-5px 0 0;">'+invTitle+'</h3> ');
    mywindow.document.write('<div class="date-meta" style="display:flex;justify-content:space-between;margin:0 0 10px;font-size:13px;"><div class="leftd" style="display:inline-block;">NO:'+cuDateRev+'</div><div class="rightd" style="display:inline-block;">'+dayName+' '+cuDate+'</div></div>');            
    mywindow.document.write(document.getElementById('example1').outerHTML);            
    mywindow.document.write(document.getElementById('example2').outerHTML);  
    mywindow.document.write('<div style="text-align: left;font-size:14px;">Revenue: '+convertNumberToWords(amount,words_string)+' Only.</div>');
    mywindow.document.write('<div class="footersign" style="text-align: center; position:relative; top:25px;width:98%;display: flex;font-weight:700;margin-top:25px; justify-content: space-between;font-size:13px; align-item:center;"><span>Sticker Section(Incharge)</span> <span>SO(AD)</span> <span>Deputy Director(Security)</span> <span>Director(Security)</span></div>');         
    mywindow.document.write('</body></html>');

    if (is_chrome) {
        setTimeout(function () { // wait until all resources loaded 
            mywindow.document.close(); // necessary for IE >= 10
            mywindow.focus(); // necessary for IE >= 10*/
            mywindow.print();
            mywindow.close();
        }, 150);
    }
    else {
        mywindow.document.close(); // necessary for IE >= 10
        mywindow.focus(); // necessary for IE >= 10*/
        mywindow.print();
        mywindow.close();
    }
    return true;
}

</script>

@endsection
